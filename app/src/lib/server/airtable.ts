...
export async function createLead(lead: Lead): Promise<string> {
  const record = await leadsTable().create({
    ...
    [F.assignedTo]: lead.assignedToId ?? "",
...
export async function updateLead(recordId: string, partial: Partial<Lead>): Promise<void> {
  ...
  if (partial.assignedToId !== undefined) fields[F.assignedTo] = partial.assignedToId ?? "";
...
export async function listLeads(): Promise<Lead[]> {
  ...
  return records.map((r) => {
    ...
    return {
      ...
      assignedToId: ((r.get(F.assignedTo) as string) || null) as string | null,
      extractedFields,
      messages,
